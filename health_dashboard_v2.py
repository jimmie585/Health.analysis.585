...
blood_pressure_filter = st.sidebar.selectbox("Select Blood Pressure", ["All"] + list(df["Blood_Pressure"].unique()))

# Filter by Diabetes
diabetes_filter = st.sidebar.selectbox("Select Diabetes", ["All"] + list(df["Diabetes"].unique()))

# Filter by Medication Adherence
medication_filter = st.sidebar.selectbox("Select Medication Adherence", ["All"] + list(df["Medication_Adherence"].unique()))

# Apply filters
if gender_filter != "All":
    df = df[df["Gender"] == gender_filter]

if blood_pressure_filter != "All":
    df = df[df["Blood_Pressure"] == blood_pressure_filter]

if diabetes_filter != "All":
    df = df[df["Diabetes"] == diabetes_filter]

if medication_filter != "All":
    df = df[df["Medication_Adherence"] == medication_filter]
...
st.sidebar.write(f"Average Age: {df['Age'].mean():.1f}")
st.sidebar.write(f"Average BMI: {df['BMI'].mean():.1f}")

# Optional: Display a bar chart for Blood Pressure distribution
st.header("Blood Pressure Distribution")
blood_pressure_counts = df["Blood_Pressure"].value_counts()
